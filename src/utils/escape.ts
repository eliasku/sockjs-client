...
let extraLookup: Record<string, string> | null = null;

function unrollLookup(escapable: RegExp): Record<string, string> {
  const unrolled: Record<string, string> = {};
  const c: string[] = [];
  for (let i = 0; i < 65536; i++) {
    c.push(String.fromCharCode(i));
  }
  escapable.lastIndex = 0;
  c.join("").replace(extraEscapable, (a: string): string => {
...
export function quote(string: string): string {
  ...
  return quoted.replace(extraEscapable, (a: string): string => {
    return extraLookup![a] ?? a;
  });
}
